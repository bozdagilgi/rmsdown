...
# Create a formatted table using gt, setting table width and font size for PDF
combined_RBM_indicators_table %>%
  gt() %>%
  tab_header(
    title = "Summary of RBM Indicators by Population Group",
    subtitle = "Table displays unweighted counts, weighted totals, and indicator values"
  ) %>%
  fmt_number(
    columns = vars(num_obs_uw, denominator),
    decimals = 0  # Remove decimals for unweighted and weighted totals
  ) %>%
  fmt_number(
    columns = vars(mean_value),
  ...
  tab_options(
    table.font.size = px(10),    # Reduce font size to fit on one page
